...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      (dist <= 160) ? pixel.l *= (1 - 0.5*dist/100) : pixel.l *= 0.2;
    }
  }
  return image;
...
PNG illinify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      // (pixel.h<=216 && (216-pixel.h)<abs(pixel.h-11))||(pixel.h>216 && (pixel.h-216)<(371-pixel.h)) ? pixel.h = 216 : pixel.h = 11;
      abs(pixel.h-216)<(pixel.h<216?abs(pixel.h-11):(371-pixel.h)) ? pixel.h = 216 : pixel.h = 11;
  ...
  for (unsigned x = 0; x < min(firstImage.width(),secondImage.width()); x++) {
    for (unsigned y = 0; y < min(firstImage.height(),secondImage.height()); y++) {
      ...
      if (secondPixel.l == 1.0) {
        firstPixel.l <=0.8 ? firstPixel.l += 0.2 : firstPixel.l = 1.0;
      }
    }
  }
  return firstImage;
